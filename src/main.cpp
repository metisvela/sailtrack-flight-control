...
// ----------------------- Boat Configuration ------------------------ //
constexpr uint8_t SERVO_PIN = 21;                 // GPIO pin for servo
...
TransferFunction cam(CAM_COEFFICIENTS);  // Now using vector
...
constexpr float MAX_ANGLE_RATE = 45.0f;           // 45°/s slew rate limit
...
void emergencyHalt() {
    flap.setAngle(0);
    while(1) { // Maintain neutral position
        digitalWrite(LED_BUILTIN, millis()%200 < 100); // Blink LED
    }
}
...
void loop() {
    ...
    // --- Sensor Acquisition ---
    uint16_t raw_angle = wand.movingAverageRawAngle(10, 1);

    // --- Height Calculation ---
    float wand_angle_deg = raw_angle * (360.0f / 4096.0f);
